...
export class EnvConfigService {
  ...
  get jwtMaxRefreshTokensPerUser(): number {
    ...
    return max ? Number.parseInt(max, 10) : 5
  }
  ...
  get port(): number {
    ...
    return port ? Number.parseInt(port, 10) : 3001
  }
  ...
  get rateLimitWindowMs(): number {
    ...
    return window ? Number.parseInt(window, 10) : 15 * 60 * 1000 // 15 minutes
  }

  get rateLimitMaxRequests(): number {
    const max = this.configService.get<string>('RATE_LIMIT_MAX_REQUESTS')
    return max ? Number.parseInt(max, 10) : 100
  }
