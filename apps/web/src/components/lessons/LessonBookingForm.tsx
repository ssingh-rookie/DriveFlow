...
export function LessonBookingForm({
  ...
  const [step, setStep] = useState<
    "details" | "time" | "payment" | "confirmation"
  >("details");
  ...
  const loading =
    createLessonMutation.isPending || instructorsLoading || servicesLoading;
  ...
  useEffect(() => {
    if (instructorsError) {
      addNotification(
        ...
        "Unable to fetch instructors from the server.",
      );
    }
    if (servicesError) {
      addNotification(
        ...
        "Unable to fetch services from the server.",
      );
    }
  }, [instructorsError, servicesError, addNotification]);
  ...
  useEffect(() => {
    if (formData.serviceId && formData.startAt) {
      ...
      if (service) {
        // Handle timezone-aware date calculation
        const startDateTime = formData.startAt; // This is already in YYYY-MM-DDTHH:MM format

        // Parse the datetime-local value correctly (treats as local time)
        ...
        console.log("=== LESSON TIME CALCULATION ===");
        console.log("Service:", service.name);
        console.log("Duration:", service.durationMinutes, "minutes");
        console.log("Start input:", startDateTime);
        console.log("Start parsed:", startDate.toString());
        console.log("End calculated:", endDate.toString());
        console.log("End output:", endDateTime);
        console.log(
          "Actual duration:",
  ...
  const handleSubmit = async () => {
    ...
    try {
      const lesson = await createLessonMutation.mutateAsync(lessonRequest);

      // Show success notification
      addNotification(
        "success",
        "Lesson Booked Successfully!",
        `Your ${services.find((s) => s.id === formData.serviceId)?.name} lesson has been booked with ${instructors.find((i) => i.id === formData.instructorId)?.displayName}.`,
      );
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Failed to book lesson";
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          {["Details", "Time", "Payment", "Confirmation"].map(
            (stepName, index) => {
              ...
              return (
                ...
                `}
                  ...
                  <span
                    className={`ml-2 text-sm ${isActive ? "text-blue-600 font-medium" : "text-gray-600"}`}
                  >
                    {stepName}
                  </span>
        ...
        <div className="space-y-6">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Leave blank to use pickup address
            </p>
          </div>
      ...
      {step === "time" && (
        <div className="space-y-6">
          ...
          {selectedService && (
            <div className="p-4 bg-blue-50 rounded-md">
              <p className="text-sm text-blue-800">
                ...
                {selectedService.durationMinutes} minutes) with{" "}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const timeValue = formData.startAt
                      ? formData.startAt.slice(11)
                      : "09:00";
                    handleInputChange(
                      "startAt",
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => {
                    const dateValue = formData.startAt
            ...
            <p className="text-sm text-gray-500 mt-1">
              📅 Select date and time separately. Available hours: 9:00 AM -
              5:00 PM (30-minute intervals)
            </p>
                  ...
                  (time) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleInputChange("startAt", suggestedDateTime)
                        }
        ...
        <div className="space-y-6">
          ...
          <div className="p-4 border-2 border-dashed border-gray-300 rounded-md text-center">
            ...
            <p className="text-sm text-gray-500">
              ...
              processing. For demo purposes, click "Book Lesson" to proceed.
            </p>
          </div>
        </div>
      )}
      ...
      {step === "confirmation" && (
        <div className="text-center space-y-6">
          ...
          <p className="text-gray-600">
            Your lesson has been booked and confirmation details have been sent
            to your email. Your instructor will contact you if any additional
            details are needed.
          </p>
          <div className="p-4 bg-green-50 rounded-md text-left">
            ...
            <div className="text-sm text-green-800 space-y-1">
              ...
              <p>
                <strong>Instructor:</strong> {selectedInstructor?.displayName}
              </p>
              <p>
              ...
              {formData.dropoffAddress && (
                <p>
                  <strong>Drop-off:</strong> {formData.dropoffAddress}
                </p>
              )}
            </div>
          </div>
