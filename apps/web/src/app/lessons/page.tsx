...
import { useLessons, useUpdateLessonStatus } from "@/hooks/useLessons";
...
export default function LessonsPage() {
  ...
  useEffect(() => {
    if (queryError) {
      addNotification(
        ...
        "Unable to fetch lessons from the server. Please refresh the page.",
      );
    }
  }, [queryError, addNotification]);
  ...
  const applyFilters = () => {
    ...
    if (filters.dateFrom) {
      ...
      filtered = filtered.filter(
        (lesson) => new Date(lesson.startAt) >= fromDate,
      );
    }
    if (filters.dateTo) {
      ...
      filtered = filtered.filter(
        (lesson) => new Date(lesson.startAt) <= toDate,
      );
    }

    // Instructor filter (for now using instructorId as the filter)
    if (filters.instructorId !== "all") {
      filtered = filtered.filter(
        (lesson) => lesson.instructorId === filters.instructorId,
      );
    }
    ...
    if (filters.searchTerm) {
      ...
      filtered = filtered.filter(
        (lesson) =>
          lesson.instructorId.toLowerCase().includes(searchLower) ||
          lesson.serviceId.toLowerCase().includes(searchLower) ||
    ...
    try {
      if (action === "cancel") {
        ...
        addNotification(
          "success",
          "Lesson Cancelled",
          `Lesson ${lessonId} has been cancelled successfully.`,
        );
      ...
      addNotification(
        "error",
        "Action Failed",
        "Unable to update lesson. Please try again.",
      );
    } finally {
  ...
  // Get unique instructors for filter dropdown with names and IDs
  const uniqueInstructors = lessons.reduce(
    (acc: Array<{ id: string; name: string }>, lesson) => {
      const existingInstructor = acc.find(
        (inst) => inst.id === lesson.instructorId,
      );
      if (!existingInstructor && lesson.instructorId) {
        acc.push({
          id: lesson.instructorId,
          name: lesson.instructorName || lesson.instructorId,
        });
      }
      return acc;
    },
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-md">
          ...
          <p className="text-sm text-blue-800">
            <strong>Persona:</strong> Student User |<strong> View:</strong> Own
            lessons only |<strong> Actions:</strong> Book, View, Cancel (with
            restrictions)
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow mb-6 p-6">
          <div className="flex flex-wrap items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-black">Filter Lessons</h2>
            <button
          ...
          {/* Top Row - Search spanning full width */}
          <div className="mb-4">
            ...
            <input
              ...
              className="w-full h-12 p-3 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-sm text-black bg-white"
            />
          </div>

          {/* Filter Row - 4 evenly spaced columns */}
            ...
            <div className="mt-6 pt-4 border-t border-gray-200">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {filters.instructorId !== "all" && (
                  <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                    Instructor:{" "}
                    {uniqueInstructors.find(
                      (inst) => inst.id === filters.instructorId,
                    )?.name || filters.instructorId}
                  </span>
                )}
                {filters.dateFrom && (
                  <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">
                ...
                <p className="text-2xl font-semibold text-gray-900">
                  {
                    lessons.filter(
                      (l) =>
                        ["confirmed", "requested"].includes(l.status) &&
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                ...
                {filteredLessons.length !== lessons.length &&
                  ` (filtered from ${lessons.length} total)`}
              </p>
            </div>
          ...
          {filteredLessons.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 mb-6">
                Get started by booking your first driving lesson!
              </p>
              <Link
                href="/lessons/book"
                    ...
                    <div className="flex-1">
                      <div className="flex items-center space-x-3 mb-2">
                        <h3 className="text-lg font-medium text-gray-900">
                          {lesson.serviceName ||
                            `Service ID: ${lesson.serviceId}`}
                        </h3>
                        <span
                          className={`px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(lesson.status)}`}
                        >
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm text-gray-600">
                        <div className="flex items-center">
                          ...
                          Instructor:{" "}
                          {lesson.instructorName || lesson.instructorId}
                        </div>
                        ...
                        <div className="flex items-center">
                          ...
                          {new Date(lesson.startAt).toLocaleDateString()} at{" "}
                      ...
                      {(lesson.status === "confirmed" ||
                        ...
                        new Date(lesson.startAt) > new Date() && (
                          <>
                            <button
                              onClick={() =>
                                handleLessonAction(lesson.id, "reschedule")
                              }
                            ...
                            <button
                              onClick={() =>
                                handleLessonAction(lesson.id, "cancel")
                              }
          ...
          {filteredLessons.length > pagination.pageSize && (
            <div className="px-6 py-4 border-t border-gray-200 flex items-center justify-between">
              <div className="text-sm text-gray-600">
                ...
                of {filteredLessons.length} results
              </div>
            ...
            <div className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-lg font-medium text-gray-900">
                      Service ID: {selectedLesson.serviceId}
                    </h3>
                    <span
                      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${getStatusColor(selectedLesson.status)}`}
                    >
                    ...
                    <div className="flex items-center">
                      ...
                      <div className="ml-3">
                        <p className="font-medium text-gray-900">
                          Instructor ID: {selectedLesson.instructorId}
                        </p>
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center">
                        ...
                        <span>
                          ...
                          at{" "}
                          {new Date(selectedLesson.startAt).toLocaleTimeString(
                            [],
                      ...
                      <div className="flex items-center">
                        ...
                        <span>
                          Duration:{" "}
                          {Math.round(
                            (new Date(selectedLesson.endAt).getTime() -
                              new Date(selectedLesson.startAt).getTime()) /
                              (1000 * 60),
                          )}{" "}
                          minutes
                        </span>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-start">
                      ...
                      <div className="ml-3">
                        ...
                        <p className="text-sm text-gray-600">
                          {selectedLesson.pickupAddress}
                        </p>
                      </div>
                    </div>
                    {selectedLesson.dropoffAddress &&
                      selectedLesson.dropoffAddress !==
                        selectedLesson.pickupAddress && (
                        <div className="flex items-start">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    <h4 className="font-medium text-gray-900 mb-3">Student</h4>
                ...
                <div className="flex items-center justify-end space-x-3 pt-4 border-t border-gray-200">
                  ...
                  {(selectedLesson.status === "confirmed" ||
                    selectedLesson.status === "requested") &&
                    new Date(selectedLesson.startAt) > new Date() && (
                      <>
                        <button
                          onClick={() => {
        ...
        <div className="mt-12 p-6 bg-gray-100 rounded-lg">
          ...
          <div className="text-sm text-gray-700 space-y-2">
            <p>
              <strong>✅ Completed Features (Task 6.2):</strong>
            </p>
            <ul className="list-disc list-inside ml-4 space-y-1">
              <li>
                Advanced filtering system (status, instructor, date range,
                search)
              </li>
              <li>Pagination with configurable page sizes</li>
              <li>
                Role-based lesson views (Student persona - own lessons only)
              </li>
              <li>Comprehensive lesson detail modal with all information</li>
              <li>
                Lesson action buttons (View, Reschedule, Cancel) with proper
                permissions
              </li>
              <li>Real-time filter feedback with active filter display</li>
              <li>
                Performance optimizations (client-side filtering and pagination)
              </li>
              <li>
                Enhanced lesson list with pricing, status indicators, and
                actions
              </li>
            </ul>
            ...
            <p className="mt-4">
              <strong>
                🔄 Ready for Task 6.3 - Basic Updates & State Management:
              </strong>
            </p>
            <ul className="list-disc list-inside ml-4 space-y-1">
            ...
            <p className="mt-4">
              <strong>
                🎯 Next: Task 6.3 - Basic Updates & State Management
              </strong>
            </p>
          </div>
        </div>
